...
describe('refresh credential decisions', () => {
  it('separates rotation, retry, expiration, and compromise', () => {
    ...
    expect(classifyRefreshCredential({ ...ACTIVE, rotatedAtMs: 9_500 })).toEqual({
      kind: 'retry-current',
    });
    expect(classifyRefreshCredential({ ...ACTIVE, rotatedAtMs: 8_000 })).toEqual({
      kind: 'revoke-family',
      reason: 'credential-reuse',
    });
    expect(classifyRefreshCredential({ ...ACTIVE, expiresAtMs: 10_000 })).toEqual({
      kind: 'reject',
      reason: 'expired',
    });
    expect(classifyRefreshCredential({ ...ACTIVE, nowMs: 10_500, rotatedAtMs: 9_500 })).toEqual({
      kind: 'revoke-family',
      reason: 'credential-reuse',
    });
  });
  ...
  it('preserves optional binding and bounded idle policy', () => {
    ...
    expect(sessionSpanMs(true, { defaultSpanMs: 1_000, rememberedSpanMs: 5_000 })).toBe(5_000);
  ...
  it('selects only the oldest excess sessions for eviction', () => {
    ...
    expect(selectSessionsForEviction(sessions, 2, (session) => session.createdAtMs)).toEqual([
      sessions[1],
    ]);
  });

  it('rejects invalid numeric policies at the public boundary', () => {
    expect(() => classifyRefreshCredential({ ...ACTIVE, gracePeriodMs: -1 })).toThrow(RangeError);
    expect(() => sessionIdleWindowMs(Number.NaN, 1_000)).toThrow(RangeError);
    expect(() => selectSessionsForEviction([], 1.5, () => 0)).toThrow(RangeError);
  });
});
