...
describe("RbacPolicy", () => {
  it("resolves transitive role and permission inheritance", () => {
    ...
    expect(
      policy.hasEveryPermission("editor", ["document:read", "document:write"]),
    ).toBe(true);
    expect(
      policy.hasAnyPermission("viewer", ["document:write", "document:read"]),
    ).toBe(true);
  });
  ...
  it("rejects duplicate, unknown, and cyclic role definitions", () => {
    expect(
      () => new RbacPolicy([{ role: "member" }, { role: "member" }]),
    ).toThrow("duplicate role");
    expect(
      () => new RbacPolicy([{ role: "member", inherits: ["missing"] }]),
    ).toThrow("unknown inherited role");
    expect(
      () =>
        new RbacPolicy([
