...
export function createGitHubProvider(config: GitHubProviderConfig): OAuthProviderClient {
  const provider = 'github';
  const clientId = requireConfig(config.clientId, 'clientId', provider);
  const clientSecret = requireConfig(config.clientSecret, 'clientSecret', provider);
  ...
  async function requestTokens(body: Record<string, string>, refresh: boolean): Promise<OAuthTokenSet> {
  ...
  return {
    ...
    async getUserInfo(tokens: OAuthTokenSet | string): Promise<OAuthUserInfo> {
      ...
      const [userResponse, emailsResponse] = await Promise.all([
        runtime.fetch(USERINFO_URL, { headers: { ...API_HEADERS, Authorization: authorization } }),
        runtime.fetch(EMAILS_URL, { headers: { ...API_HEADERS, Authorization: authorization } }),
      ]);
      await requireOk(userResponse, provider, 'USERINFO_FAILED', 'User info request');
      ...
      if (emailsResponse.ok) {
        ...
        try {
          ...
          throw new OAuthError('GitHub returned invalid email data', provider, 'USERINFO_FAILED');
        }
      ...
      if (email === undefined) {
        throw new OAuthError('No email found on GitHub account', provider, 'NO_EMAIL');
      }
      ...
      if (id === undefined) {
        throw new OAuthError('GitHub response is missing id', provider, 'USERINFO_FAILED');
      }
