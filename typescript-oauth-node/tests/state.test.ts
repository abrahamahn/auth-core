...
import { createOAuthStateManager, createPkcePair, OAuthStateError } from '../src/index.js';
...
describe('OAuth state manager', () => {
  it('owns generic protected state without knowing application payload fields', () => {
    ...
    const manager = createOAuthStateManager<{ readonly linking: boolean }>({
      ...
      unprotect: (protectedState) => Buffer.from(protectedState, 'base64url').toString('utf8'),
      parsePayload(value) {
    ...
    const state = manager.create('google', 'https://app.example/callback', { linking: true });
    expect(manager.decode(manager.encode(state))).toEqual(state);

    now += 10_001;
    expectStateError(() => manager.decode(manager.encode(state)), 'EXPIRED_STATE');
  });
  ...
  it('rejects malformed and unprotected state with stable failure categories', () => {
    ...
    expectStateError(() => manager.decode('{"provider":"google"}'), 'MALFORMED_STATE');
...
function expectStateError(operation: () => unknown, code: OAuthStateError['code']): void {
  try {
    operation();
