...
describe("GitHub provider", () => {
  ...
  it("reports provider errors without copying provider response bodies into logs", async () => {
    ...
    await expect(
      provider.exchangeCode("code", "https://app.example/callback"),
    ).rejects.not.toThrow("secret-bearing upstream body");
  });
});
...
describe("Apple provider", () => {
  it("retains id_token, verifies its signature and claims, and caches public keys", async () => {
    const clientSigningKey = generateKeyPairSync("ec", {
      namedCurve: "P-256",
    }).privateKey.export({
      type: "pkcs8",
      format: "pem",
    ...
    const idToken = createAppleIdToken(
      identityKeys.privateKey.export({ type: "pkcs8", format: "pem" }),
    );
    const fetchMock = vi.fn<typeof fetch>();
    fetchMock
      .mockResolvedValueOnce(
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          keys: [{ ...publicJwk, kid: "apple-key", use: "sig", alg: "RS256" }],
        }),
      );
  ...
  it("requires the identity token instead of confusing it with the access token", async () => {
    ...
    const accessOnly: OAuthTokenSet = {
      accessToken: "access",
      tokenType: "Bearer",
    };
...
function createAppleIdToken(privateKey: string): string {
  const header = Buffer.from(
    JSON.stringify({ alg: "RS256", kid: "apple-key" }),
  ).toString("base64url");
