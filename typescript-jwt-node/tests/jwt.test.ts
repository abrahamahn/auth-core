...
describe("HS256 JWT adapter", () => {
  it("signs, decodes, and verifies object payloads", () => {
    ...
    expect(verify(token, CURRENT_SECRET, { currentTimeSeconds: NOW })).toEqual(
      decode(token),
    );
  });
  ...
  it("rejects invalid duration, clock, secret, and payload inputs", () => {
    ...
    expect(() => sign({}, "", { issuedAtSeconds: NOW })).toThrow(
  ...
  it("rejects malformed structure, headers, payloads, and signatures with stable codes", () => {
    ...
    const valid = sign({ sub: "user-123" }, CURRENT_SECRET, {
      issuedAtSeconds: NOW,
    });
  ...
  it("enforces expiration boundaries and bounded tolerance", () => {
    ...
    expect(() =>
      verify(token, CURRENT_SECRET, { currentTimeSeconds: NOW + 59 }),
    ).not.toThrow();
    expectJwtError(
  ...
  it("never treats decoded content as verified content", () => {
    const token = sign({ role: "user" }, CURRENT_SECRET, {
      issuedAtSeconds: NOW,
    });
...
describe("secret rotation", () => {
  ...
  it("signs with the current secret and accepts the previous secret during migration", () => {
    const current = sign({ sub: "current" }, CURRENT_SECRET, {
      issuedAtSeconds: NOW,
    });
    const previous = sign({ sub: "previous" }, PREVIOUS_SECRET, {
      issuedAtSeconds: NOW,
    });

    expect(
      verifyWithRotation(current, config, { currentTimeSeconds: NOW })["sub"],
    ).toBe("current");
    expect(
      verifyWithRotation(previous, config, { currentTimeSeconds: NOW })["sub"],
    ).toBe("previous");
    expect(
      checkTokenSecret(current, config, { currentTimeSeconds: NOW }).usedSecret,
    ).toBe("current");
    expect(
      checkTokenSecret(previous, config, { currentTimeSeconds: NOW })
        .usedSecret,
    ).toBe("previous");
  });

  it("does not retry malformed or expired tokens with the previous secret", () => {
    const expired = sign({}, PREVIOUS_SECRET, {
      expiresIn: 1,
      issuedAtSeconds: NOW,
    });
    const result = checkTokenSecret(expired, config, {
      currentTimeSeconds: NOW + 2,
    });
  ...
  it("provides a configured facade without exposing key material", () => {
    ...
    expect(handler.verify(token, { currentTimeSeconds: NOW })["sub"]).toBe(
      "user-123",
    );
    expect(
      handler.checkSecret(token, { currentTimeSeconds: NOW }).usedSecret,
    ).toBe("current");
