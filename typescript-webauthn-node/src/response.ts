...
function requiredString(record: Record<string, unknown>, key: string, label: string): string {
  ...
  if (typeof value !== 'string' || value === '') throw invalidResponse(`${label} is required`);
  return value;
}
...
function credentialType(value: unknown): 'public-key' {
  if (value !== 'public-key') throw invalidResponse('credential type must be public-key');
  return value;
}

function authenticatorAttachment(value: unknown): AuthenticatorAttachment | undefined {
...
function clientExtensionResults(value: unknown): AuthenticationExtensionsClientOutputs {
  if (value === undefined) return {};
  if (!isRecord(value)) throw invalidResponse('clientExtensionResults must be an object');
  return value;
}

function transports(value: unknown): AuthenticatorTransportFuture[] | undefined {
  if (value === undefined) return undefined;
  if (!Array.isArray(value) || !value.every((entry) => TRANSPORTS.has(entry as never))) {
    throw invalidResponse('credential transports are invalid');
  }
  return value as AuthenticatorTransportFuture[];
}

export function parseWebAuthnRegistrationResponse(value: unknown): RegistrationResponseJSON {
  ...
  if (
    ...
    (!Number.isSafeInteger(publicKeyAlgorithm) || typeof publicKeyAlgorithm !== 'number')
  ) {
  ...
  return {
    ...
    response: {
      ...
      attestationObject: requiredString(response, 'attestationObject', 'attestation object'),
      ...(authenticatorData === undefined ? {} : { authenticatorData }),
      ...(responseTransports === undefined ? {} : { transports: responseTransports }),
      ...(publicKeyAlgorithm === undefined ? {} : { publicKeyAlgorithm }),
      ...(publicKey === undefined ? {} : { publicKey }),
    },
    clientExtensionResults: clientExtensionResults(value['clientExtensionResults']),
    type: credentialType(value['type']),
    ...(attachment === undefined ? {} : { authenticatorAttachment: attachment }),
  };
}

export function parseWebAuthnAuthenticationResponse(value: unknown): AuthenticationResponseJSON {
  if (!isRecord(value) || !isRecord(value['response'])) {
  ...
  return {
    ...
    response: {
      clientDataJSON: requiredString(response, 'clientDataJSON', 'client data'),
      authenticatorData: requiredString(response, 'authenticatorData', 'authenticator data'),
...
export function readWebAuthnResponseTransports(value: unknown): readonly AuthenticatorTransportFuture[] {
