...
describe('AuthWebAuthnServer', () => {
  ...
  it('rejects unsafe or mismatched relying-party configuration', () => {
    expect(() =>
      createAuthWebAuthnServer({ ...config, expectedOrigin: 'http://login.example.com' }),
    ).toThrow(AuthWebAuthnError);
    expect(() =>
      createAuthWebAuthnServer({ ...config, expectedOrigin: 'https://unrelated.test' }),
    ).toThrow(/rpId/u);
  });
});
...
describe('WebAuthn browser response normalization', () => {
  it('normalizes registration and authentication responses', () => {
    ...
    ).toMatchObject({ id: 'credential-a', response: { signature: 'signature' } });
  });
