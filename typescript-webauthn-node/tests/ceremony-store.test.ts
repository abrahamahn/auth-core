...
describe("InMemoryWebAuthnCeremonyStore", () => {
  it("consumes a ceremony exactly once", () => {
    let nowMs = 1_000;
    const store = new InMemoryWebAuthnCeremonyStore({
      ttlMs: 500,
      now: () => nowMs,
    });
    ...
    expect(() => store.consume("reg:user-1", "registration")).toThrow(
      AuthWebAuthnError,
    );
    nowMs = 2_000;
  });
  ...
  it("deletes expired and mismatched ceremonies before returning an error", () => {
    ...
    expect(() => store.consume("auth:one", "registration")).toThrow(
    ...
    nowMs = 1_500;
    expect(() => store.consume("auth:two", "authentication")).toThrow(
      /expired/u,
    );
    expect(store.size).toBe(0);
  });
